(async () => {
    ...
    // SSO 生成地址
    var ssoUrl = "ssoIndex.do?apOu=GuidanceApp_LDAP&datetime1=" + new Date().getTime();
    try {
        let response = await fetch(ssoUrl);
        let html = await response.text();
        // 提取 Token
        let sessionMatch = html.match(/name=["']?sessionId["']?[\s\S]*?value=["']?([^"'\s>]+)["']?/i);
        let userMatch = html.match(/name=["']?userid["']?[\s\S]*?value=["']?([^"'\s>]+)["']?/i);
        let data = {
            sessionId: sessionMatch ? sessionMatch[1] : "NOT_FOUND",
            userid: userMatch ? userMatch[1] : "NOT_FOUND",
            // 完整的 CURL 指令，包含所有必要的 headers
            curl_command: sessionMatch ? `curl -i -s -k -X POST 'https://ap.itc.ntnu.edu.tw/GuidanceApp/StdtLoginCtrl?PageType=1B' \\
